...
function prepareBatch(config, cb) {
    var pattern = config.get("glob"),
        unit = "files";
    ...
    if (pattern && !config.get("files")) {
        glob(pattern, function (err, matches) {
            if (matches) {
                ...
                error("Found", matches.length, unit, "to test.");

                cb(config);
            } else {
                panic("Glob pattern", pattern, "matched zero files.");
            }
        });
    } else {
        cb(config);
...
exports.route = function (argv) {
    ...
    if (parsedArgs.hub === true) {
        error("Command-line Hub option does not appear to be a valid URL. Ignoring.");
