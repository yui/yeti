...
;var SimpleEvents = (function () {
    ...
    function SimpleEvents(socket, debug) {
        ...
        function onIncoming(message) {
            ...
            if (data.event) {
                self.debug("got ev", data.event, "queueUntil:", self.queueUntilEvent);
        ...
        function onClose(cxn) {
            if (self.socket.removeListener) {
                ...
                self.debug("removing data listener");
    ...
    proto.emit = function (event, args) {
        ...
        if (this.queueUntilEvent) {
            this.debug("queueing for transport later");
    ...
    proto.invokeRPC = function (event, args) {
        ...
        self.debug("invokeRPC for " + event);
        self.debug("self.listeners", self.listeners);
    ...
    proto.queueUntil = function (event) {
        this.debug("queue events until we get " + event);
        this.queueUntilEvent = event;
    };

    proto.flushQueue = function () {
        this.debug("flushing event queue");
        var i,
            message,
            self = this,
        ...
        if (length) {
            for (i = 0; i < length; i += 1) {
                ...
                self.debug("sending queued message", message);
