...
Hub.prototype._onTowerLog = function (loglevel, message) {
    ...
    this.emit("log." + loglevel + ".socketio", message);
}
...
Hub.prototype._onTowerConnection = function (socket) {
    ...
    socket.on("data", function (message) {
        try {
            ...
            self.debug("Bad JSON from agent, ignoring.");
            return;
        }
        ...
        self.agentManager.connectAgent(id, message.ua, socket);

        console.log("SockJS register done");
...
Hub.prototype._createRouter = function () {
    ...
    router.path(/\/public\/(.*)/, function () {
        this.get(function (file) {
            ...
            if (/\.js$/.test(file)) {
                files.unshift(path.join(dir, "sockjs-0.3.min.js"));
            }
