...
var HubBatchManager = require("./batch").HubBatchManager;
...
var Hub = module.exports = function Hub(options) {
    ...
    this.batchManager = new HubBatchManager(this.agentManager);
...
Hub.prototype._onRunConnection = function (socket) {
    ...
    socket.on("register", function (registration) {
        ...
        console.log("connect");
        self.pool.connectAgent(id, registration.ua, socket);
...
Hub.prototype._createSocketIO = function (server) {
    ...
    if (self.log && self.log.log) {
        self.emit(["log", "debug"], "Setting up Socket.io to use Winston.");
        socketOptions.logger = self.log;
    }
