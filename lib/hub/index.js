...
Hub.prototype._setupAgentPoolEvents = function () {
    ...
    self.blizzard.on("session", function (blizzard) {
        ...
        // FIXME We need a better way to disconnect
        // events when the BlizzardSession disconnects.

        function onAgentConnect(agent) {
            blizzard.request("agentConnect", agent.getName());
        }

        function onAgentDisconnect(agent) {
            blizzard.request("agentDisconnect", agent.getName());
        }

        self.pool.on("agentConnect", onAgentConnect);
        self.pool.on("agentDisconnect", onAgentDisconnect);

        blizzard.once("end", function () {
            self.pool.removeListener("agentConnect", onAgentConnect);
            self.pool.removeListener("agentDisconnect", onAgentDisconnect);
        });
    });
};
