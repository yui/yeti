    ...
    router.path(/\/public\/(.*)/, function () {
        this.get(function (file) {
            ...
            this.res.streamFiles([__dirname + "/view/public/" + file], {
                // FIXME: Only call $yetify for inject.js.
                // Guarded for now to prevent script errors.
                postpend: "if (window.$yetify) { $yetify(" + JSON.stringify({
 ...
 */
Hub.prototype.attachServer = function () {
    var args = Array.prototype.slice.apply(arguments),
        augmentServer = args.shift(),
        route = args.shift(),
        options = {};
    ...
    this.hubListener = new HubListener(this, augmentServer, options);
