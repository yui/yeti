...
BlizzardSession.prototype.bindEvents = function () {
    ...
    // Handle RPC calls.
    this.on("rpc.*", function routeRPC(/* args ... cb */) {
        var method = this.event.substr(4); // remove "rpc."
...
BlizzardSession.prototype.bridge = function (emitter, event) {
...
BlizzardSession.prototype.onMessage = function (id, message) {
    ...
    if (message.method) {
        ...
        if (self.listeners(event).length) {
            self.emit(event, params, onComplete);
        } else {
            self.emit("fail", id,
