...
function BlizzardSession(id, socket, instigator) {
...
BlizzardSession.prototype.outgoingBridge = function (emitter, event) {
    emitter.on(event, this.emit.bind(this, "rpc." + event));
};

BlizzardSession.prototype.incomingBridge = function (emitter, event) {
    this.on("request." + event, emitter.emit.bind(emitter, event));
};
...
BlizzardSession.prototype.callRequestCallback = function (id, simpleError, result) {
    assert(id);
