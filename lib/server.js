...
function serveExpress (port, path, cb) {
    ...
    app.put("/tests/add", function (req, res) {
        if (!req.body.tests.length) return res.send("Expected to find tests.", 500);
        ...
        if (Object.keys(socket.clients).length) {
            tests.emit("add", id, urls);
        } else {
            return res.send("Unable to add: no browsers are listening.", 500);
        }
